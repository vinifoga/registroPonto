...
Uri urlEmployee =
    Uri.parse("https://registro-ponto-api-v2.herokuapp.com/colaboradores");
...
class _EmployeeUpdateState extends State<EmployeeUpdate> {
  ...
  void initState() {
    ...
    if(widget.employee != null){
      _editedEmployee = widget.employee;
      nameController.text = _editedEmployee!.nome ?? "";
      mailController.text = _editedEmployee!.email ?? "";
      birthDateController.text = _editedEmployee!.dataNascimento ?? "";
      status = _editedEmployee!.ativo;
      phoneNumberController.text = _editedEmployee!.telefone;
      dropdownValueJobTitle = jobTitles.firstWhere((jobTitle) => jobTitle.id==_editedEmployee!.cargo.id).descricao;
      admissionDateController.text = _editedEmployee!.dataAdmissao ?? "";
      cpfController.text = _editedEmployee!.cpf ?? "";
      pisController.text = _editedEmployee!.pis ?? "";
      startWorkController.text = _editedEmployee!.horaEntra ?? "";
      endWorkController.text = _editedEmployee!.horaSai ?? "";
      breakTimeController.text = _editedEmployee!.intervaloTempo ?? "";
      dropdownValueUnit = units.firstWhere((unit) => unit.codUnidade==_editedEmployee!.unidadeOrganizacional.codUnidade).descricao;
    }
  }

  late final jobTitleKey = GlobalKey();
  late List<String> jobTitlelist =
      jobTitles.map((e) => e.descricao.toString()).toList();
  late String dropdownValueJobTitle = jobTitlelist.first;

  late List<String> unitsList =
      units.map((e) => e.descricao.toString()).toList();
  late String dropdownValueUnit = unitsList.first;

  bool validateAndSave() {
    final form = formKey.currentState;
    final jobTitle = jobTitles
        .firstWhere((element) => element.descricao == dropdownValueJobTitle);
    final unit =
        units.firstWhere((element) => element.descricao == dropdownValueUnit);
  ...
  void validateAndSubmit() async {
    if (validateAndSave()) {
      ...
      try {
        JobTitle jobTitleChoose = jobTitles
            .where((jobTitle) => jobTitle.descricao == dropdownValueJobTitle)
            .first;
        OrganizationUnit unitChoose =
            units.where((unit) => unit.descricao == dropdownValueUnit).first;

        Uri urlUpdateEmployee = Uri.parse(urlEmployee.toString()+'/'+_editedEmployee!.matricula.toString());
        ...
        var responseUser =
            await http.put(urlUpdateEmployee, headers: header, body: body);
                          ...
                          Container(
                            ...
                            child: Padding(
                              ...
                              child: DropdownButton<String>(
                                ...
                                items: unitsList.map<DropdownMenuItem<String>>(
                                    (String value) {
